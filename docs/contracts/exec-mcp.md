...
| | |
|---|---|
| **Purpose** | 定義 Exec Tool Provider 的最小 argv 輸入輸出契約 |
| **Audience** | Exec MCP 實作者、企業客戶 |
| **Source of truth** | 本文件是 **Exec MCP 整合契約**的權威來源 |
| **Related** | [contracts/case-api.md](case-api.md)、[operations/policy.md](../operations/policy.md)、[guides/mcp-providers.md](../guides/mcp-providers.md) |
...
所有「能不能跑」由 Agent 在 **呼叫 MCP 之前** 決定。使用者編輯 **`config/policy.yaml`**（見 [operations/policy.md](../operations/policy.md)）；編譯後由 `core/mcp_policy.py` 執行檢查。
...
LLM **不**決定是否安全；只決定語意與工具規劃。安全由 `policy.yaml` 編譯後的確定性規則處理（見 [operations/policy.md](../operations/policy.md)）。
...
- Case / Portal 串接：[case-api.md](case-api.md)
- Agent policy：`config/policy.yaml`（見 [operations/policy.md](../operations/policy.md)）
