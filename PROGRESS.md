...
|------|------|
| 產品階段 | **PoC / 內部驗證** |
| 架構對齊 | **Sprint 4 完成** — Connector 抽象（`connectors/` + `CasePortalConnector`） |
| 核心流程 | Connector poll → Understanding → Decision → LangGraph → Connector 回覆 |
| 測試 | `make test` — 148 tests OK |
| 下一 sprint | **Sprint 5 — Domain 分離**（尚未開始） |
...
- [x] `workflow/runner.py`、`workflow/graph.py` 改依賴 `Connector` 而非 `CasePortalBridge`
...
| 優先 | 項目 | 備註 |
|------|------|------|
| 高 | **Sprint 5：Domain 分離** | 見 [06-architecture-alignment-plan.md](docs/architecture/06-architecture-alignment-plan.md) |
...
| 項目 | 說明 | 建議處理時機 |
|------|------|--------------|
| runtime bootstrap 在 `main.py` | MCPRegistry / WorkflowDeps 組裝仍在入口 | 可選；Sprint 5 前評估 |
...
| 領域邏輯在 workflow 內 | collection / investigation 在 `graph.py` | **Sprint 5** |
